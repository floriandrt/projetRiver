  ...
  : _imgItem(0), _maskItem(0), _undoStackPos(0)
{
  ...
  /*
  QImage* _energy = new QImage(QSize(500,500),QImage::Format_RGB32);
  _energy->fill(QColor(0,0,255));

  stateBox = new StateBox(_scene, _energy);
  stateBox->setZValue(100);
  _scene->setBackgroundBrush(palette().light());

  stateBox->setPos(0,0);
  _scene->addItem( stateBox);
  */
  ...
  connect(_resizeWidget.editionMode, SIGNAL(stateChanged(int)), this, SLOT(updateCursor()));
  connect(_resizeWidget.edgeDetector, SIGNAL(activated(int)), this, SLOT(updateView()));
  connect(_resizeWidget.finishNew, SIGNAL(clicked()), this, SLOT(generateStatebox()));
...
void MainWindow::newFile(){
  delete _scene;
  _scene = new ImageScene(this);
  connect(_scene, SIGNAL(mouseMoved(QPointF, QPointF)), this, SLOT(paintMask2(QPointF, QPointF)));
  connect(_scene, SIGNAL(mousePressed(QPointF)), this, SLOT(test(QPointF)));
  connect(_scene, SIGNAL(mouseReleased(QPointF)), this, SLOT(test2(QPointF)));
//  _newPix = QPixmap(500, 500);
//  _newPix.fill(Qt::black);
  _newImage = new QImage(QSize(500,500),QImage::Format_RGB32);
  _newImage->fill(Qt::black);
  _newItem = _scene->addPixmap(QPixmap::fromImage(*_newImage));
  //_maskItem = (stateBox->_scene)->addPixmap(_maskPix);
  _newItem->setZValue(1000); //Always in front of imgItem

  cerr << "AVANT SET SCENE" << endl;

//    QImage _test(QSize(500,500),QImage::Format_RGB32);
//    _test.fill(QColor(0,0,0));
    _scene->setBackgroundBrush(palette().dark());
...
{
    cerr << "ENTREE OPEN IMAGE" << endl;
  _img = image;
  delete _scene;
  //stateBox->_scene = new ImageScene(this);
  _scene = new ImageScene(this);

  QImage* _energy = new QImage(QSize(500,500),QImage::Format_RGB32);
  _energy->fill(QColor(0,0,255));

  stateBox = new StateBox(_scene, _energy, new Ui::ResizeWidget(_resizeWidget), "", _paintedLines);
  stateBox->setZValue(100);
  _scene->setBackgroundBrush(palette().light());
  ...
  _scene->setBackgroundBrush(palette().dark());
  //(stateBox->_scene)->setBackgroundBrush(palette().dark());
  //_imgItem = (stateBox->getScene())->addPixmap(QPixmap::fromImage(_img));
  //_imgItem->setZValue(1);
    cerr << "ARRIVE ICI" << endl;
  if(!mask.isNull())
  {
    _maskPix = mask;
  }else{
    _maskPix = QPixmap(500, 500);
    _maskPix.fill(Qt::transparent);
  }
  _maskItem = _scene->addPixmap(_maskPix);
  //_maskItem = (stateBox->_scene)->addPixmap(_maskPix);
  _maskItem->setZValue(1000); //Always in front of imgItem
  _maskItem->setOffset(16,16);


  stateBox->setMaskItem(_maskItem);
  stateBox->setMaskPix(&_maskPix);
  cerr << "AVANT SET SCENE" << endl;
  _view->setScene(_scene);
  cerr << "APRES SET SCENE" << endl;
  ...
  _viewGaussianBlur->setEnabled(true);
  cerr << "APRES ENABLE TRUE" << endl;
  ...
  updateActions();
  cerr << "APRES UPDATE ACTIONS" << endl;
  updateCursor();
  cerr << "SORTIE OPEN IMAGE" << endl;
}
...
void MainWindow::cairResize(int newWidth, int newHeight)
{
  ...
  {
    CAIR_HD( &source, &source_weights, newWidth, newHeight, conv, ener, &dest_weights, &dest, updateCallback, NULL);
  }
  if(prog.wasCanceled())
    return;
  QImage newImg = CMLtoQImage(dest);
...
void MainWindow::test(QPointF pos){
    cerr << "event test" << endl;
    _startingPoint = pos;
}

//Point crop(double min, double max, QPointF start, QPointF end){
//    Point res(4);
//    Point seg1(4);
//    seg1[0] = start.x();
//    seg1[1] = start.y();
...
//        seg2[2] = min;
//        seg2[3] = max;
//        inter = Point::intersection(seg1,seg2);
//        res[0] = min;
//        res[1] = inter[1];
//    }else if(start.x() > max){
//        seg2[0] = max;
...
void MainWindow::test2(QPointF pos){
    cerr << "event test2" << endl;
    if(_startingPoint.x() < pos.x()){
        _paintedLines.push_back(QLine(_startingPoint.x(),_startingPoint.y(),pos.x(),pos.y()));
    }else{
        _paintedLines.push_back(QLine(pos.x(),pos.y(),_startingPoint.x(),_startingPoint.y()));
    }
}

void MainWindow::paintMask2(QPointF oldPos, QPointF newPos)
{
//    cerr << "PAINT MASK2" << endl;
    QPainter qPaint;
    qPaint.begin(_newImage);
    qPaint.setBrush(Qt::NoBrush);
    qPaint.setPen(Qt::black);
    qPaint.drawLine(_startingPoint,oldPos);
    qPaint.setPen(Qt::blue);
    for(int i = 0; i<(int)_paintedLines.size(); i++){
        qPaint.drawLine(_paintedLines[i]);
    }
    qPaint.drawLine(_startingPoint,newPos);
    qPaint.end();

    _newItem->setPixmap(QPixmap::fromImage(*_newImage));

}

void MainWindow::paintMask(QPointF oldPos, QPointF newPos)
{
    if(_resizeWidget.editionMode->isChecked()){
        cerr << "PAINT MASK" << endl;
        QPointF offset(-16,-16);
      ...
      painter.drawLine(oldPos+offset, newPos+offset);
      _maskItem->setPixmap(_maskPix);
...
{
    cerr << "CHANGE VIEW" << endl;
    QImage* _image = stateBox->getImage();
    if(_image == NULL){
        cerr << "image null" << endl;
    }
  if(view == _viewImage)
  {
    stateBox->setImage();
    //_imgItem->setPixmap(QPixmap::fromImage(*_image));
    return;
  }
  cerr << "APRES VIEW VIEW IMAGE" << endl;
  CML_color source(_image->width(), _image->height());
  CML_color dest(_image->width(), _image->height());
  QImagetoCML(*_image,source);
  CAIR_convolution conv = V1;
  switch( _resizeWidget.edgeDetector->currentIndex() )
  {
    case 0 : conv = V1; break;
  ...
  if(view == _viewVEnergy)
    CAIR_V_Energy( &source, conv, ener, &dest, NULL );
  if(view == _viewHEnergy)
    CAIR_H_Energy( &source, conv, ener, &dest, NULL );
  if(view == _viewGaussianBlur)
    CAIR_GaussianBlur( &source, conv, ener, &dest, NULL);
  cerr << "APRES CAIR" << endl;
  stateBox->changeView(CMLtoQImage(dest));
  //_imgItem->setPixmap(QPixmap::fromImage(CMLtoQImage(dest)));
  cerr << "CHANGE VIEW FIN" << endl;
}

void MainWindow::generateStatebox(){
  cerr << "generateStatebox" << endl;
  delete _scene;
  //stateBox->_scene = new ImageScene(this);
  _scene = new ImageScene(this);
...
void MainWindow::updateCursor()
{
    if(_resizeWidget.editionMode->isChecked()){
      ...
      stateBox->setCursor( QCursor(pix) );
      //_imgItem->setCursor( QCursor(pix) );
      //setCursor( QCursor(pix) );
    }else{
       stateBox->setEditionBool(true);
       stateBox->setCursor( Qt::ArrowCursor );
    }
}
...
void MainWindow::createActions()
{
  ...
  _viewHEnergy->setEnabled(true);
  _viewGaussianBlur = new QAction(tr("View Gaussian base blur"), this);
  _viewGaussianBlur->setShortcut(tr("Ctrl+K"));
  _viewGaussianBlur->setCheckable(true);
  _viewGaussianBlur->setEnabled(true);
  ...
  _viewGroup->addAction(_viewGaussianBlur);
...
void MainWindow::createMenus()
{
  ...
  _viewMenu->addAction(_viewGaussianBlur);
